...
// These need to be persistent so they can always be accessed, even if gdt.cpp is never used again
static struct gdt gdt_table;
static struct tss tss_table;

// Global so asm_lgdt can access it
struct gdt_desc gdt_descriptor;

// Functions declared in assembly
...
void gdt_init() {
    ...
    // Now we setup the TSS Descriptor
    // TSS Descriptor
